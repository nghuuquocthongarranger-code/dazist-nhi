...
const WesternHero3D = lazy(() => import("../WesternHero3D").then((m) => ({ default: m.WesternHero3D })));

export function WesternHero({
  onSelectTopic,
  hideLabels = false,
}: {
  ...
  return (
    <section className="relative h-[100svh] min-h-[640px] w-full overflow-hidden flex items-start justify-center pt-32 sm:pt-40">
      <Suspense fallback={<div className="absolute inset-0 bg-cosmic" />}>
        <WesternHero3D onSelectTopic={onSelectTopic} hideLabels={hideLabels} />
      </Suspense>
      ...
      <div className="relative z-10 px-6 text-center max-w-3xl mx-auto pointer-events-none">
        ...
        >
          Mỗi ngôi sao trong chòm sao là một chủ đề luận giải — chạm vào để khám phá.
        </motion.p>
      </div>
    </section>
  );
